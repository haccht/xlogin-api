...
class PoolsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create pool" do
    assert_difference('Pool.count') do
      post pools_url, params: { pool: { idle: @pool.idle, name: @pool.name, size: @pool.size, template: @pool.template } }
    end
  ...
  test "should update pool" do
    patch pool_url(@pool), params: { pool: { idle: @pool.idle, name: @pool.name, size: @pool.size, template: @pool.template } }
